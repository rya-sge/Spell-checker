    ...
    SECTION("Resize hashmap") {
        ...
        SECTION("Reduce") {
            ...
            for(int N = hm->size() - 1; N >= 0; --N){
                ...
                if(factor <= hm->getMinFactor()){ // realloc expected
                    double actualFactor = (double)hm->size() / hm->max_size();
...
template<typename T>
void testString(HashMapWrapper2<T> *hm){
...
template<typename T>
void testSize_t(HashMapWrapper2<T> *hm){
...
TEST_CASE("Linear Probing", "[hashmap]")
{
   ...
   SECTION("Size_T values") {
        ...
        auto* hw2 = reinterpret_cast<HashMapWrapper2<size_t> *>(&lb2);
        testSize_t(hw2);
    }
    ...
    SECTION("String values") {
        ...
        auto* hw2 = reinterpret_cast<HashMapWrapper2<std::string> *>(&lb2);
        testString(hw2);
       /* HashMapSeparateChaining<string> sc;
        auto* hwSc = reinterpret_cast<HashMapWrapper2<string> *>(&sc);
...
TEST_CASE("SeparateChaining", "[hashmap]"){
    SECTION("Size_T values") {
        ...
        auto* hwSc = reinterpret_cast<HashMapWrapper2<size_t> *>(&sc);
        bar(hwSc, values, valuesExclude);

        HashMapLinearProbing<size_t> sc2;
        auto* hw2 = reinterpret_cast<HashMapWrapper2<size_t> *>(&sc2);
        testSize_t(hw2);
    }
    ...
    SECTION("String values") {
        ...
        vector<string> valuesExclude = {"h", "tulaS", "Non", "Enjoy"};
        HashMapSeparateChaining<string> sc;
        auto* hw = reinterpret_cast<HashMapWrapper2<std::string> *>(&sc);
        bar(hw, values, valuesExclude);

        HashMapSeparateChaining<string> sc2;
        auto* hw2 = reinterpret_cast<HashMapWrapper2<std::string> *>(&sc2);
        testString(hw2);
        /* HashMapSeparateChaining<string> sc;
         auto* hwSc = reinterpret_cast<HashMapWrapper2<string> *>(&sc);
         bar(hwSc, values, valuesExclude);*/
